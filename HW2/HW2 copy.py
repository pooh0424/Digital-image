...
if __name__ == '__main__':
    for i in range(1, 3):
        ...
        result = cv2.equalizeHist(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY))
        histfrequency = calhis(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)) #計算直方圖
        img = Equalization(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY),histfrequency)
        print(result-img)
